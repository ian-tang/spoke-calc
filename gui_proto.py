...
class Draw(Frame):
    ...
    def initUI(self):
        ...
        for i in range(len(spokes_drive)):
            hub_coords_drive.append([flange_diameter/2 * sin(spokes_drive[i] * pi/180), flange_diameter/2 * cos(spokes_drive[i] * pi/180)])
            rim_coords_drive.append([effective_rim_diameter/2 * sin(spokes_drive[i] * pi/180), effective_rim_diameter/2 * cos(spokes_drive[i] * pi/180)])
        for i in range(len(spokes_nondrive)):
            hub_coords_nondrive.append([flange_diameter/2 * sin(spokes_nondrive[i] * pi/180), flange_diameter/2 * cos(spokes_nondrive[i] * pi/180)])
            rim_coords_nondrive.append([effective_rim_diameter/2 * sin(spokes_nondrive[i] * pi/180), effective_rim_diameter/2 * cos(spokes_nondrive[i] * pi/180)])
        ...
        for i in range(len(spokes_drive)):
            ...
            canvas.create_oval(center[0]+x-oval_size, center[1]-y-oval_size, center[0]+x+oval_size, center[1]-y+oval_size, fill="black")
        for i in range(len(spokes_nondrive)):
            x = hub_coords_nondrive[i][0]
            y = hub_coords_nondrive[i][1]
            canvas.create_oval(center[0]+x-oval_size, center[1]-y-oval_size, center[0]+x+oval_size, center[1]-y+oval_size, fill="red")
